...
const CopyClipboard = (props) => {
    ...
    return (
        <div>
            <div className=' flex justify-center items-center transition-all duration-1000 px-4 py-3 w-full md:w-1/2 bg-green-100 rounded-lg m-auto'>
                <input type="text" value={newUrl} className='w-full bg-transparent' name="" id="" disabled />
                <button><img src={iscopied ? check : clipboard} className='h-6 w-6 ml-1 hover:scale-105 transition-all duration-300' alt="" onClick={() => {
                    copy(newUrl)
                    setIsCopied(true)
                    setTimeout(() => {
                        setIsCopied(false)
